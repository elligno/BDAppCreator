...
void TextExtractor::Init( const char* pszInput )
{
    ...
    PdfMemDocument document( pszInput );
		PdfVecObjects& w_check = document.GetObjects();
		size_t w_sizObj = w_check.GetSize();
    ...
    for( int i=0; i<nCount; i++ ) 
    {
        PdfPage* pPage = document.GetPage( i );
				...
				// some check
				PdfRect w_pageSiz = pPage->GetPageSize();
				const PdfRect w_mediaBox =  pPage->GetMediaBox();

        //this->ExtractText( &document, pPage );
				ExtractTextJB(&document, pPage);
    }
}
		...
		PdfFont* pCurFont, const PdfString & rString )
{
    ...
    PdfString unicode = pCurFont->GetEncoding()->ConvertToUnicode( rString, pCurFont );
		const pdf_utf16be* w_asd = unicode.GetUnicode();
		const char* w_ttStr = unicode.GetString();
//		rString.GetUnicode();
    const char* pszData = unicode.GetStringUtf8().c_str();

		QByteArray w_testBarr(pszData);
		std::cout << w_testBarr.data() << std::endl;
		QString w_utf8 = QString::fromUtf8(pszData);
		std::cout << w_utf8.data() << std::endl;
// 		int len = MultiByteToWideChar(CP_UTF8,0,pszData,-1,NULL,0);
// 		std::vector<wchar_t> w_buff(len);
// 		MultiByteToWideChar(CP_UTF8, 0, pszData,-1,&w_buff[0], w_buff.size());

//		std::string w_checkStr = unicode.GetStringUtf8(); //jb
		while( *pszData ) {
      // printf("%02x", static_cast<unsigned char>(*pszData) );
			std::cout << std::hex << static_cast<unsigned char>(*pszData) << "\n"; // jb
       ++pszData;
    }
	printf("(%.3f,%.3f) %s \n", dCurPosX, dCurPosY, unicode.GetStringUtf8().c_str() );
}

void TextExtractor::ExtractTextJB(PdfMemDocument* pDocument, PdfPage* pPage)
{
	const char* pszToken = nullptr;
	EPdfContentsType eType;
	PdfVariant var;
	PdfContentsTokenizer tokenizer(pPage);
	bool   bTextBlock = false;
					...
					if( var.IsArray())
					{
						...
						for (size_t i = 0; i < a.GetSize(); i++)
						{
							if (a[i].IsString())
							{
								...
								QByteArray w_check(utf8.c_str());
								QString w_str = QString::fromUtf8(w_check.data());
